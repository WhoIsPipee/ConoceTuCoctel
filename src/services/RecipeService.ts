...
export async function getRecipes(filters: SearchFilter){
    ...
    const {data} = await axios(url)
    const result = DrinksAPIResponse.safeParse(data) //safeParse Valida datos de forma segura , (Necesita un data para poder ejecutar )
    if(result.success){
        return result.data
    }
}
...
// IMPORTANTE: La URL necesitaba el protocolo de HTTPS
